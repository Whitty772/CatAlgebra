...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = {
        ...
        currentSeason: 'primavera', // Estación actual: 'primavera', 'verano', 'otono', 'invierno'.
    };
    ...
    function createDirtPattern() {
        ...
        switch (state.currentSeason) {
            case 'primavera':
                seasonColors = ['#8B4513', '#A0522D', '#556B2F', '#6B8E23']; // Marrones y verdes
                break;
            case 'verano':
                seasonColors = ['#C2B280', '#D2B48C', '#BDB76B', '#F0E68C']; // Tonos secos y amarillentos
                break;
            case 'otono':
                seasonColors = ['#D2691E', '#A0522D', '#8B4513', '#CD853F']; // Naranjas y ocres
                break;
            case 'invierno':
                seasonColors = ['#FFFFFF', '#F0F8FF', '#E6E6FA', '#DCDCDC']; // Blancos y grises para la nieve
                break;
            default:
                seasonColors = ['#8B4513', '#A0522D', '#654321', '#D2691E'];
        }

        for (let i = 0; i < 10000; i++) {
            const x = Math.random() * patternCanvas.width;
            const y = Math.random() * patternCanvas.height;
            const color = seasonColors[Math.floor(Math.random() * seasonColors.length)];
    ...
    function drawPlants() {
        plants.forEach(p => {
            const growth = p.growthStage / 100; // 0.0 to 1.0

            // --- Tallo (Stem) ---
            const stemHeight = 15 * growth;
            const stemWidth = 3 * growth;
            ctx.fillStyle = '#654321'; // Brown color for the stem
            ctx.fillRect(p.x - stemWidth / 2, p.y - stemHeight, stemWidth, stemHeight);

            // --- Hojas (Leaves) ---
            const leafSize = 4 * growth;
            ctx.fillStyle = 'green';

            // Number of leaves depends on growth stage
            const numLeaves = Math.floor(growth * 6);

            for (let i = 0; i < numLeaves; i++) {
                // Distribute leaves along the stem
                const yOffset = (i / numLeaves) * stemHeight * 0.8;
                // Alternate sides
                const xOffset = (i % 2 === 0 ? -1 : 1) * (stemWidth + leafSize/2);
                ctx.fillRect(p.x + xOffset - leafSize / 2, p.y - yOffset - leafSize, leafSize, leafSize);
            }

            // --- Frutos (Fruits) ---
            if (growth > 0.6) { // Fruits appear after 60% growth
                const fruitGrowth = (growth - 0.6) / 0.4; // Normalize fruit growth from 0 to 1
                const fruitSize = 6 * fruitGrowth;
                const numFruits = Math.floor(fruitGrowth * 3);

                // Set fruit color based on crop type
                let fruitColor = 'white';
                switch (p.type) {
                    case 'arroz': fruitColor = `rgba(218, 165, 32, 1)`; break; // Dorado
                    case 'papa': fruitColor = `rgba(139, 69, 19, 1)`; break;   // Marrón
                    case 'fresa': fruitColor = `rgba(255, 0, 0, 1)`; break;     // Rojo
                    case 'maiz': fruitColor = `rgba(255, 255, 0, 1)`; break;    // Amarillo
                    case 'tomate': fruitColor = `rgba(255, 99, 71, 1)`; break;   // Rojo tomate
                    case 'lechuga': fruitColor = `rgba(0, 128, 0, 1)`; break;     // Verde oscuro - lechuga doesn't have fruits, but let's keep it green
                    default: fruitColor = 'white';
                }
                ctx.fillStyle = fruitColor;

                for (let i = 0; i < numFruits; i++) {
                    const yOffset = (i / numFruits) * stemHeight * 0.5 + stemHeight * 0.3;
                    const xOffset = (i % 2 === 0 ? -1 : 1) * (stemWidth + fruitSize);
                    ctx.fillRect(p.x + xOffset - fruitSize / 2, p.y - yOffset - fruitSize, fruitSize, fruitSize);
                }
            }
        });
    }
    ...
    function updateSimulationState() {
        ...
        if (state.currentMode === 'watering') {
            ...
            if (state.soilHumidity < 100) {
                state.soilHumidity += 2.0; // Tasa de absorción de agua
            }
        ...
        // 3. Gestionar Crecimiento de Plantas según la Estación
        if (state.soilHumidity > 30 || state.currentSeason === 'invierno') {
            plants.forEach(p => {
                switch (state.currentSeason) {
                    case 'primavera':
                        // Crecimiento rápido
                        if (p.growthStage < 100) {
                            p.growthStage += 5;
                        }
                        break;
                    case 'verano':
                        ...
                        if (p.growthStage < 100) {
                            p.growthStage += 2.5; // Crece más lento que en primavera
                        }
                        break;
                    case 'otono':
                        // El crecimiento se detiene, la planta se mantiene madura
                        break;
                    case 'invierno':
                        // La planta se marchita y muere
                        if (p.growthStage > 0) {
                            p.growthStage -= 2; // Se reduce el crecimiento
                        }
                        break;
                }
                // Asegurarse de que el valor de crecimiento esté siempre entre 0 y 100
                p.growthStage = Math.max(0, Math.min(100, p.growthStage));
            });
        }
    }
    ...
    function plantCrops() {
        ...
        for (let i = 0; i < 100; i++) { // Plantar 100 cultivos
    ...
    seasonSelector.addEventListener('change', (e) => {
        ...
        dirtPattern = createDirtPattern(); // Regenerate pattern on season change
    });
