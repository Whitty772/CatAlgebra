...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawPlants() {
        plants.forEach(p => {
            const maxSize = 12; // Tamaño máximo del cuadrado
            const size = (p.growthStage / 100) * maxSize;

            if (p.growthStage < 100) {
                // El color es siempre verde durante el crecimiento
                ctx.fillStyle = `rgba(0, 255, 0, ${p.growthStage / 100})`;
            } else {
                // Colores basados en el tipo de cultivo para plantas maduras
                switch (p.type) {
                    case 'arroz': ctx.fillStyle = `rgba(218, 165, 32, 1)`; break; // Dorado
                    case 'papa': ctx.fillStyle = `rgba(139, 69, 19, 1)`; break;   // Marrón
                    case 'fresa': ctx.fillStyle = `rgba(255, 0, 0, 1)`; break;     // Rojo
                    case 'maiz': ctx.fillStyle = `rgba(255, 255, 0, 1)`; break;    // Amarillo
                    case 'tomate': ctx.fillStyle = `rgba(255, 99, 71, 1)`; break;   // Rojo tomate
                    case 'lechuga': ctx.fillStyle = `rgba(0, 128, 0, 1)`; break;     // Verde oscuro
                    default: ctx.fillStyle = 'white';
                }
            }

            // Dibuja un rectángulo (cuadrado) centrado en la posición de la planta
