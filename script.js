...
const audioIntroStart = document.getElementById('audio-intro-start');
...
// Mapa de dificultades a sus elementos de audio
const musicMap = {
    EASY: audioMusicEasy,
...
// Función para detener toda la música de fondo
function stopAllMusic() {
    Object.values(musicMap).forEach(audio => {
        if (audio) {
            audio.pause();
            audio.currentTime = 0;
        }
...
// Función para reproducir música de fondo para una dificultad específica
function playBackgroundMusic(difficultyKey) {
    stopAllMusic(); // Detener cualquier otra música
    const musicToPlay = musicMap[difficultyKey];
    if (musicToPlay) {
        musicToPlay.play();
    }
}
...
function startGame(difficultyKey) {
    playBackgroundMusic(difficultyKey);
