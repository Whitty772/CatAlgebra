...
let attackTimeouts = []; // Almacena los IDs de los timeouts de ataque
...
function clearAllAttackTimeouts() {
    attackTimeouts.forEach(timeoutId => clearTimeout(timeoutId));
    attackTimeouts = []; // Limpiar el array
}

function stopTimer() {
    clearInterval(timerInterval);
    timerInterval = null;
    clearAllAttackTimeouts(); // Asegura que no queden ataques programados
}
...
function startGame(difficultyKey) {
    ...
    // Lógica para mostrar el minijuego y programar ataques solo en modo Extremo
    if (difficultyKey === 'VERY_HARD') {
        dodgeMinigameContainer.classList.remove('hidden');
        scheduleAttacks(); // Programar los ataques precisos
    } else {
        dodgeMinigameContainer.classList.add('hidden');
    }
...
// --- LÓGICA DE ATAQUES PROGRAMADOS PARA EL MODO EXTREMO ---
function scheduleAttacks() {
    const attackTimestamps = [
        "0:32", "0:42", "0:43", "0:44", "0:45", "1:02", "1:03", "1:05", "1:06",
        "1:35.60", "1:35", "1:36.25", "1:36.90", "1:37.50", "1:38", "1:38.80",
        "1:39.25", "1:39.75", "1:45", "1:45.70", "1:46.50", "1:47", "1:47.60",
        "1:48.20", "1:48.70", "1:49", "1:59.80", "2:00", "2:00.80", "2:01.30",
        "2:02", "2:02.60", "2:03", "2:03.60", "2:09", "2:14", "2:18.80", "2:20",
        "2:21", "2:22.50", "2:23.60", "2:24.70", "2:26", "2:27.20"
    ];

    const totalGameTime = difficultyTimes['VERY_HARD'] * 1000;

    attackTimestamps.forEach(timeStr => {
        const parts = timeStr.split(/[:.]/);
        const minutes = parseInt(parts[0], 10);
        const seconds = parseInt(parts[1], 10);
        const milliseconds = parts.length > 2 ? parseInt(parts[2].padEnd(3, '0'), 10) : 0;

        const timeFromStart = (minutes * 60 + seconds) * 1000 + milliseconds;

        // El tiempo del evento es desde el inicio de la música/juego.
        // El temporizador de la UI va hacia atrás, así que no podemos usar `timeLeft`.
        // Calculamos el retraso desde el momento en que se llama a `startGame`.
        if (timeFromStart > 0) {
            const timeoutId = setTimeout(() => {
                triggerAttack();
            }, timeFromStart);
            attackTimeouts.push(timeoutId);
        }
    });
}
...
function triggerAttack() {
    ...
    // Elige un carril aleatorio para el atacante y el disparo
    const attackLane = Math.random() < 0.5 ? 'top' : 'bottom';
    setAttackerLane(attackLane);
    ...
    projectile.classList.add(attackLane === 'top' ? 'lane-top' : 'lane-bottom');
    dodgeMinigameContainer.appendChild(projectile);
    projectile.classList.add('firing');

    // Detección de colisión y daño
    setTimeout(() => {
        if (playerLane === attackLane) {
            handleHitEffects();
            loseHealth(); // El jugador pierde vida si es golpeado
        }
    ...
    // Permitir el siguiente ataque después de un breve enfriamiento
    setTimeout(() => {
        isAttackSequenceRunning = false;
    }, 500);
}

// Los listeners para los botones de prueba han sido eliminados
